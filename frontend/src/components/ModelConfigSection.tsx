...
/** 模型配置区(ModelConfig CRUD)。从 /models 页抽出,供设置页「模型配置」Tab 复用。 */
  ...
  return (
    <div className="space-y-5">
              ...
              <div className="flex items-start justify-between gap-4 flex-wrap">
                <div className="min-w-0">
                  <div className="flex items-center gap-2 mb-2 flex-wrap">
                    ...
                    {m.is_default && <Badge tone="success" dot>默认</Badge>}
      ...
      <Modal
        ...
        footer={
          <>
            ...
            <Button variant="primary" onClick={create} loading={creating}>创建</Button>
          </>
        }
      >
        <div className="space-y-4">
          <Field label="配置名称" htmlFor="m-name">
            <Input id="m-name" value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} placeholder="如:内网视觉模型" />
          </Field>
          <Field label="能力类型">
          ...
          <Field label="API Key" hint="留空则不设;加密保存,界面脱敏。" htmlFor="m-key">
            <Input id="m-key" type="password" value={form.api_key} onChange={(e) => setForm({ ...form, api_key: e.target.value })} placeholder="sk-..." />
          </Field>
        </div>
      </Modal>
      ...
      <Modal
        ...
        size="sm"
        footer={
          <ConfirmFooter destructive confirmText="删除" loading={deleting} onCancel={() => setDeleteTarget(null)} onConfirm={confirmDelete} />
        }
      />
    </div>
  );
